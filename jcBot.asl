...
+!talk <- 
	...
	!waitAnswer;
	!say("Iv·n","Incorpora la relacion entre pakistan y islamabad al mapa capital de inmediato");
	!waitAnswer;
	...
	!waitAnswer;
	!say("Iv·n","QuÈ tal te fuÈ el ˙ltimo curso? Aprobaste todas las materias?");
	!waitAnswer;
...
+recibida(Answer) : .substring("cine",Answer) <- 
	!say("Juan Carlos","Eres m·s de series o de pel√≠culas?");
	!waitAnswer;
	.abolish(recibida(_)).
...
+recibida(Answer) : .substring("NO",Answer) <- 
	!say("Juan Carlos","Este curso imparte ProgramacionI. Quien dices que te di√≥ clase en ProgramacionI?");
	!waitAnswer.
